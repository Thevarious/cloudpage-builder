...
const functionalityFilesInput = document.getElementById("functionalityFiles");
...
const generatedOutputPanel = document.getElementById("generatedOutputPanel");
...
form.addEventListener("submit", async function (event) {
  ...
  // Console file log
  logSelectedFiles(designFilesInput, "Design Files");

  logSelectedFiles(functionalityFilesInput, "Functionality Files");

  // Send files to backend
  const response = await fetch("/analyze-design", {
    method: "POST",
    body: formData,
  });
  ...
  // Show analysis
  analysisOutput.textContent = JSON.stringify(result.analysis, null, 2);

  console.log("Design Analysis JSON:");
...
generateBtn.addEventListener("click", async function () {
  ...
  const successTitle = document.getElementById("successTitle").value;

  const successMessage = document.getElementById("successMessage").value;

  const showSuccessIcon = document.getElementById("showSuccessIcon").checked;

  // Collect dynamic fields
  const fields = [];

  document.querySelectorAll("#fieldRows > div").forEach((row) => {
    fields.push({
  ...
  console.log("Generated CloudPage HTML:");
...
downloadHtmlBtn.addEventListener("click", function () {
  const blob = new Blob([generatedHtmlOutput.value], { type: "text/html" });
  ...
  link.download = "generated-cloudpage.html";
...
copyHtmlBtn.addEventListener("click", async function () {
  await navigator.clipboard.writeText(generatedHtmlOutput.value);
